...
############################################################################
    def datos_keyword(self,imagen, keyword):
        ...
        if keyword == "ALL":
            key=hduhdr.ascardlist()
            print key
        else:
            key=hduhdr.ascardlist()[keyword]
            #print key
        hdufile.close()
        return key
############################################################################
...
############################################################################
    def crear(self, datos, x, y, outfile):
        print 'crea fits old',x,y,outfile
        #ojo quitar
        #self.old_lib=True
        ...
        print "pyfits old?",self.old_lib

        if self.old_lib:
            np = numpy.frombuffer(datos, dtype=numpy.int16)
            np=np-32768
        ...
        if self.old_lib:
            ...
            header.update(key="bitpix", value=16, comment="array data type")
            header.update('NAXIS1', x, "length of data axis 1")
            header.update('NAXIS2', y, "length of data axis 2",after='NAXIS1')
            print "fits: naxis1",x,"naxis2",y

            header.update('BZERO', 32768, 'BZERO')
            header.update('BSCALE', 1, 'BSCALE')
...
############################################################################
    def appendHeader(self, image,x,y):
        ...
        key=hdr.ascardlist()['NAXIS1']
        print 'NAXIS1',key

        key=hdr.ascardlist()['NAXIS2']
        print 'NAXIS2',key


        hdr.update('NAXIS', 2)
        hdr.update('NAXIS1', x, "length of data axis 1")
        hdr.update('NAXIS2', y, "length of data axis 2",after='NAXIS1')
        hdr.update('BZERO', 32768, 'BZERO')
        hdr.update('BSCALE', 1, 'BSCALE')
        #print len(self.k)
        for j in range(len(self.k)):
                hdr.update(self.k[j][0],self.k[j][1],self.k[j][2])

        #print "gian",self.mi_ccd.gain
        #print "filter",self.mi_ccd.mi_filtro.filtro
        try:
                hdr.update("FILTER",self.mi_ccd.mi_filtro.filtro, "Filter", after = 'GAINMODE')
        except:
                hdr.update("FILTER",self.mi_ccd.mi_filtro.filtro, "Filter")

        if not self.mi_ccd.mi_filtro.angulo is None:
                hdr.update("POLANGLE",self.mi_ccd.mi_filtro.angulo, "Polarimetro angle", after = 'FILTER')
        if self.instrument=='Polima2' :
            print "Si esta Polima2"
            hdr.update("POL2LIN",self.mi_ccd.mi_filtro.mesa_pos, "Polima2 Linear Position", after = 'FILTER')

        hdr.update("SECONDAR", self.mi_ccd.mi_secundario.foco,  self.m, after = 'ALTITUDE')
        #lo de la temp
        if self.mi_ccd.can_readtemp:
                hdr.update("CCDTEMP",self.mi_ccd.temp, "CCD temperarure (celsius degree)",after="EXPTIME")

        #lo extra
        if self.mi_ccd.extra_header:
                #print "si hay parametros extras del header"
                self.mi_ccd.update_extra_header()
                #hdr.update("CCDSIZE",self.mi_ccd.CCDSIZE, "Physical CCD Size")
                #hdr.update("DATASEC",self.mi_ccd.DATASEC, "CCD Virtual Section")
                #hdr.update("CCDSEC",self.mi_ccd.CCDSEC, "CCD Section")
                hdr.update("BIASSEC",self.mi_ccd.BIASSEC, "CCD BIAS Section")
                hdr.update("TRIMSEC",self.mi_ccd.TRIMSEC, "Data section containing usefuldata")
        else:
        ...
        #hdr.update("NAMPS",n, 'Number of Amplifiers')
        #hdr.update("CCDNAMPS",used, 'Number of amplifiers used')
        hdr.update("AMPNAME",self.mi_ccd.lista_output[self.mi_ccd.output_actual][0], 'Amplifier name')

        #Voy a poner aqui los casos particulares, para sus clases
        self.mi_ccd.local_header(hdr)
        #################################

        hdr.update("FILENAME",image.split('/')[-1], "Original Host Filename")
        hdr.update("CREATOR","Python Oan ccds", "Name of the software task that created the file")
        hdr.update("VERSION",self.version, "Application Software Version")
...
############################################################################
    def upgrade_fits_header(self,outfile):
        ...
        if self.instrument=='Polima2' :
            print "Si esta Polima2"
            hdr.update("POL2LIN",self.mi_ccd.mi_filtro.mesa_pos, "Polima2 Linear Position")
...
############################################################################
    def myinfo(self):
        print "en clase vieja de bin2fits"
############################################################################
#f=BIN2FITS()
#f.test()
'''fits=PYF()
